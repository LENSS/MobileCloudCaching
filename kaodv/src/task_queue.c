...
task *get_task()
{
    ...
    if (task_q != NULL)
    {
        printk(KERN_ERR "TASK_QUEUE: Error with task queue\n");
    }
    queue_unlock();
    return NULL;
}
...
int insert_task(int type, struct sk_buff *packet)
{
    ...
    switch (type)
    {
        ...
        case TASK_RREP:
            ...
            memcpy(&(new_task->src_hw_addr), &(((struct ethhdr *)skb_mac_header(packet))->h_source), sizeof(unsigned char) * ETH_ALEN);
            break;
